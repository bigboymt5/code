...
def update_next_jobs():
    ...
    job_names = [f"t_fx.py ({ticker})" for ticker in 외환쌍목록]
...
def get_next_run_time(job_name):
    """각 작업의 다음 실행 시간을 정확하게 계산 (5분 간격으로 15개 외환쌍 실행)"""
    current_time = datetime.now()
...
def setup_schedules():
    """설정 변수를 기반으로 스케줄 설정 (5분 간격으로 15개 외환쌍 실행)"""
    logger.info(f"스케줄 설정 시작 - 오전: {오전실행}시, 오후: {오후실행}시, 주말실행: {주말실행}, 실행간격: {실행간격}분")

    # 15개 외환쌍에 대해 순차적으로 스케줄 등록
    for idx, ticker in enumerate(외환쌍목록):
        # 각 외환쌍의 시작 분 계산 (5분 간격)
        ...
        job_key = f"t_fx.py ({ticker})"
        args = f"h_val=12 h_tic={ticker}"

        # 주말 실행 여부에 따라 스케줄 등록
        if 주말실행:
            # 매일 실행
            schedule.every().day.at(오전시간).do(schedule_job, "t_fx.py", args, job_key)
            schedule.every().day.at(오후시간).do(schedule_job, "t_fx.py", args, job_key)
            logger.info(f"{ticker} 스케줄 등록 완료 - 매일 {오전시간}, {오후시간}")
        else:
            # 평일만 실행
            for day in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday']:
                getattr(schedule.every(), day).at(오전시간).do(schedule_job, "t_fx.py", args, job_key)
                getattr(schedule.every(), day).at(오후시간).do(schedule_job, "t_fx.py", args, job_key)
            logger.info(f"{ticker} 스케줄 등록 완료 - 평일 {오전시간}, {오후시간}")

    logger.info("모든 스케줄 등록 완료")
