...
def update_next_jobs():
    ...
    # 파일 이름 변경: news.py -> win_news.py, h2.py -> win_h2.py, ppx.py 삭제
    job_names = ["win_news.py", "win_h2.py"]
...
def get_next_run_time(job_name):
    ...
    # 파일명과 실행 시간 변경
    if job_name == "win_news.py":
        # 매시 5분 (01:05, 02:05, ... 23:05)
        ...
        # 00시에는 실행하지 않음
        if next_time.hour == 0:
            next_time = next_time.replace(hour=1)
            
        return next_time

    elif job_name == "win_h2.py":
        # 매시 10분 (01:10, 02:10, ... 23:10)
        next_time = base_time.replace(minute=10)
        if next_time <= current_time:
            next_time += timedelta(hours=1)
...
def setup_schedules():
    ...
    # win_news.py - 매시 5분에 실행 (01:05 ~ 23:05)
    for hour in range(1, 24):  # 1시부터 23시까지
        schedule.every().day.at(f"{hour:02d}:05").do(schedule_job, "win_news.py")

    # win_h2.py - 매시 10분에 실행 (01:10 ~ 23:10)
    for hour in range(1, 24):  # 1시부터 23시까지
        schedule.every().day.at(f"{hour:02d}:10").do(schedule_job, "win_h2.py")
