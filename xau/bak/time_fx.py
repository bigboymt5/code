...
def get_next_run_time(job_name):
    ...
    if job_name == "fx.py":
        ...
        # 현재 시간 이후의 실행 시간이 없으면 다음 시간대로 넘어감
        if next_minute is None:
            next_time = base_time.replace(minute=0) + timedelta(hours=1)
        else:
            next_time = base_time.replace(minute=next_minute)
...
def setup_schedules():
    # news.py - 매시 정각 15분에 실행 (주석처리)
    # schedule.every().hour.at("15:00").do(schedule_job, "news.py")

    # h2.py - 매시 정각 16분에 실행 (주석처리)
    # schedule.every().hour.at("16:00").do(schedule_job, "h2.py")

    # fx.py - 매시 정각 10분에 실행 (20분으로 시간 변경)
    schedule.every().hour.at("00:00").do(schedule_job, "fx.py")
    #schedule.every().hour.at("10:00").do(schedule_job, "fx.py")
    schedule.every().hour.at("20:00").do(schedule_job, "fx.py")
    #schedule.every().hour.at("30:00").do(schedule_job, "fx.py")
    schedule.every().hour.at("40:00").do(schedule_job, "fx.py")
    #schedule.every().hour.at("50:00").do(schedule_job, "fx.py")
